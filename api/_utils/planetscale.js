...
export const insertGameEvent = async ({ ref, game_ref, type, sent_by, ...data }) => await createConnection.execute(
  "INSERT INTO `game_events` (`ref`, `game_ref`, `type`, `sent_by`, `sent_at`, `data`) VALUES (?, ?, ?, ?, ?, ?)",
  [ref, game_ref, type, sent_by, new Date(), data]
);

export const selectGameEventsForGameRef = async (game_ref) => await createConnection.execute(
